...
class DashboardController extends Controller
{
    ...
    public function statisticsData()
    {
        ...
        |-----------------------------------*/
        $totalWorkorder = 0;
        $totalActiveWorkorder = 0;

        $workorderTable = Schema::hasTable('work_orders') ? 'work_orders' : (Schema::hasTable('workorders') ? 'workorders' : null);

        if ($workorderTable) {
            $totalWorkorder = DB::table($workorderTable)
                ->where('status', 'active')
                ->count();

            $totalActiveWorkorder = DB::table($workorderTable)
                ->where('is_closed', 0)
                ->where('status', 'active')
                ->count();
        }
        ...
        |-----------------------------------*/
        return [
            'total_workorder'               => (int) $totalWorkorder,        // Mbps
            'active_workorder'              => (int) $totalActiveWorkorder,
    ...
    public function clientSummary()
    {
        ...
        $total_outstanding   = ($total_invoice_due ?? 0);
        $previous_month_due  = ($pm_invoice_due ?? 0);
        $current_month_due   = ($cm_invoice_due ?? 0);
    ...
    {
        $accounts = DB::table('fund_account_balance')->get();

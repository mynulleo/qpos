...
class haxe_Http
{
    public function __construct($url)
    {
        if (! isset($this->onStatus)) {
            $this->onStatus = [new _hx_lambda([&$this, &$url], 'haxe_Http_0'), 'execute'];
        }
        if (! isset($this->onError)) {
            $this->onError = [new _hx_lambda([&$this, &$url], 'haxe_Http_1'), 'execute'];
        }
        if (! isset($this->onData)) {
            $this->onData = [new _hx_lambda([&$this, &$url], 'haxe_Http_2'), 'execute'];
        }
    ...
    {
        return call_user_func_array($this->onStatus, [$status]);
    }
    ...
    {
        return call_user_func_array($this->onError, [$msg]);
    }
    ...
    {
        return call_user_func_array($this->onData, [$data]);
    }
    ...
    {
        $this->file = _hx_anonymous(['param' => $argname, 'filename' => $filename, 'io' => $file, 'size' => $size]);
    }
    ...
    public function request($post)
    {
        ...
        $old = (isset($this->onError) ? $this->onError : [$this, 'onError']);
        $err = false;
        $this->onError = [new _hx_lambda([&$err, &$me, &$me1, &$old, &$output, &$post], 'haxe_Http_5'), 'execute'];
    ...
    public static function requestUrl($url)
    {
        $h = new haxe_Http($url);
        $r = null;
        $h->onData = [new _hx_lambda([&$h, &$r, &$url], 'haxe_Http_6'), 'execute'];
        $h->onError = [new _hx_lambda([&$h, &$r, &$url], 'haxe_Http_7'), 'execute'];
        $h->request(false);

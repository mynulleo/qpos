...
class com_wiris_plugin_impl_HttpImpl extends haxe_Http
{
    public function __construct($url, $listener)
    {
        if (! isset($this->onError)) {
            $this->onError = [new _hx_lambda([&$this, &$listener, &$url], 'com_wiris_plugin_impl_HttpImpl_0'), 'execute'];
        }
        if (! isset($this->onData)) {
            $this->onData = [new _hx_lambda([&$this, &$listener, &$url], 'com_wiris_plugin_impl_HttpImpl_1'), 'execute'];
        }

                    ...
                    <fieldset>
                        ...
                        <div class="row g-3">
                            <Select title='Work Order' v-model='data.workorder_id' field='data.workorder_id'
                                ...
                                placeholder='--Select One--' :closeOnSelect='true' :required='true' col="7"
                                @change="getWorkorderInfo" />
                            <date-picker id='date2' v-model='data.challan_date' field='data.challan_date'
                                title='Challan Date' placeholder='Challan Date' col='3' :req='true'></date-picker>
                            <Input v-model='data.delivery_location' field='data.delivery_location'
                                title='Delivery Location' :req='false' col="12" />
                    ...
                    <fieldset>
                        ...
                        <div class="row">
                            <Input v-model='data.receive_by' field='data.receive_by' title='Name' :req='false'
                                col="4" />
                            <Input v-model='data.receive_by_designation' field='data.receive_by_designation'
                                title='Designation' :req='false' col="4" />
                            <Input v-model='data.receive_by_mobile' field='data.receive_by_mobile' title='Mobile'
                                :req='false' col="4" />
                        </div>
                    </fieldset>
                </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in data.challan_details" :key="index">
                            <td>
                                <Select v-model="item.item_id" field="item.item_id" label="title"
                                    :reduce="(obj) => obj.id" :options="items" placeholder="--Select One--"
                            ...
                            <td>
                                <Input v-model="item.receive_qty" field="item.receive_qty" :readonly="!$route.params.id"
                                    col="12" />
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="multiple_fields_actions_btn d-flex align-items-center gap-2">
                                    ...
                                    <button v-if="isLastItem(data.challan_details, index)" type="button"
                                        class="btns add_more" data-bs-toggle="tooltip" data-bs-placement="top"
                                        data-bs-title="Add" v-x-tooltip @click.prevent="addChallanDetailsRow()">

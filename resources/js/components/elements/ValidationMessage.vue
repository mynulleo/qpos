      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title text-black" id="validateModalLabel">
            You need to chock the following errors before proceed
          </h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <div v-if="$root.validation_errors && Object.keys($root.validation_errors).length > 0
          " class="col-12 py-2 mb-3" style="background: #fddede">
            <div class="error p-2">
              <div v-for="(err, errIndex) in $root.validation_errors" :key="errIndex" class="text-danger">
                {{ err[0] }}
                <br />
              </div>
            </div>

<template>
    ...
    >
        <p class="number">
            {{ table.meta.from }} - {{ table.meta.to }} of
            {{ table.meta.total }}
        </p>
        <ul class="list-unstyled">
            ...
            >
                <a
                    ...
                    @click="get_datas(table.meta.current_page - 1)"
                >
                    <svg
            ...
            <template v-for="leftNum in decrement()" :key="leftNum">
                <li v-if="leftNum > 0">
                    <a href="javascript:void(0)" @click="get_datas(leftNum)">{{
                        leftNum
                    }}</a>
                </li>
            </template>
            ...
            <li class="active">
                <a
                    href="javascript:void(0)"
                    @click="get_datas(table.meta.current_page)"
                    >{{ table.meta.current_page }}</a
                >
            </li>
            ...
            <template v-for="rightNum in 6" :key="rightNum">
                <li
                    v-if="
                        table.meta.current_page + rightNum <=
                        table.meta.last_page
                    "
                >
                    <a
                        href="javascript:void(0)"
                        @click="get_datas(table.meta.current_page + rightNum)"
                        >{{ table.meta.current_page + rightNum }}</a
                    >
                </li>
            </template>

            <li v-if="table.meta.current_page < table.meta.last_page">
                <a
                    href="javascript:void(0)"
                    @click="get_datas(table.meta.current_page + 1)"
                >
